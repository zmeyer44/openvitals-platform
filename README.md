...
The parser registry records explicit supported, unsupported, empty, review-needed, and error decisions. CSV labs and PDF labs both materialize observations; PDFs run through the Vercel AI Gateway (`AI_GATEWAY_API_KEY` required) using a structured-output extractor and land as `trust_level = ai_extracted`. When the gateway key is unset, when the PDF exceeds the byte cap, or when extraction returns no observations, the import falls back to the placeholder review task. Image files still classify into review-needed placeholder tasks until a real extractor is available.
